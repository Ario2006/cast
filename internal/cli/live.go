...
func newLiveCommand() *cobra.Command {
	return &cobra.Command{
		...
		Long: "Serve a project directory as a read-only, expiring local-network session.\n\n" +
			"Examples:\n  cast live .\n  cast live ./project\n\n" +
			"Serving an existing local service by port is the next milestone.",
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if _, err := strconv.Atoi(args[0]); err == nil {
				return apperror.New("Live port proxying is not available yet.", "For now, provide a project directory, such as: cast live .")
			}
